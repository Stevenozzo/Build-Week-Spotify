...
function playSong(...trackUri) {
    fetch('https://api.spotify.com/v1/me/player/play', {
        ...
        body: JSON.stringify({
            uris: trackUri,//Cosa voglio riprodurre 
            device_id: localStorage.getItem('device_id'),//Su quale dispositivo ? 
            play: true//Se voglio avviare la riproduzione oppure fare altro
        })
    })
        .then(response => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const api = async () => {
        ...
        const res1 = await fetch('https://api.spotify.com/v1/me', options);//Ottengo i dati dell'utente corrente
        const data1 = await res1.json();//qui ci sono le info sullo user
        console.log(data1)
        document.getElementById('username').innerText = data1.display_name;//Mostro il nome dell'utente
